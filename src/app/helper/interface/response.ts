import { swipeBtn } from "src/app/modules/shared/helper/dataModal/shared-interface";
import { fileCompData } from "../core.data.interface";
import { userPermission } from "../dataModal/user";
import { swipeItemData } from "../shared-interface";
...
export declare type tableAction = 'ADD' | 'EDIT' | "PRINT" | 'DELETE' | 'VIEW' | 'INACTIVE' | 'ACTIVE' | 'EXPORT' | 'RELIVE';
export interface tableColum {
    ...
    isPrimary?: boolean;
    icon_append?: icon_append,
    colType?: 'TEXT' | 'DATE' | 'CURRENCY' | 'IMAGE' | 'DATETIME' | 'STAFF_EMP_ID' | 'SPONSORID' | 'DROPDOWN' | 'CHILD_ID_BASIC' | 'CHILD_ID' | 'VIEW_INFO' | 'VIEW_TAG_INFO';
...
export interface dataBuilder extends swipeItemData {
    column: dataColum[];
    swipeBtn: swipeBtn[];
    name: string;
    isLazy: boolean;
